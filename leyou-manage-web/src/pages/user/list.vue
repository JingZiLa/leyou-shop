<template>
  <v-card>
    <v-toolbar class="elevation-0">
      <v-btn color="primary" @click="addGoods">新增会员测试</v-btn>
      <v-spacer/>
      <v-flex xs3>
        状态：
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="delGoods(props.item)">
            <i class="el-icon-delete"/>
          </v-btn>
          <v-btn icon v-if="props.item.saleable" @click="goodsSoldOut(props.item)">下架</v-btn>
          <v-btn icon v-else @click="goodsSoldOut(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    data() {
      return {
        ...
        headers: [
          ...
          {text: '会员头像', align: 'center', sortable: false, value: 'title'},
          {text: '会员名称', align: 'center', sortable: false, value: 'cname'},
          {text: '手机号码', align: 'center', value: 'bname', sortable: false,},
   ...
   /* mounted() { // 渲染后执行
    ...
    methods: {
      ...
      goodsSoldOut(item) {
        ...
        this.$message.confirm('是否确定'+ (item.saleable ? "下架" : "上架") +'该商品?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$http.get("/item/goods/soldOut/"+item.id).then(() => {
            this.$message.success('已'+ (item.saleable ? "下架" : "上架") +'该商品');
            // 重新加载数据
            this.getDataFromServer();
          }).catch(() => {
            this.$message.error('商品'+ (item.saleable ? "下架" : "上架") +'失败');
          })
        }).catch(()=>{
        })
      },
      delGoods(item) {
          this.$message.confirm('此操作将永久删除该商品，是否继续?', '提示', {
            confirmButtonText: '确定删除',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.$http.get("/item/goods/del/"+item.id).then(() => {
